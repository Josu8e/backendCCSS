...
export class PresupuestoComponent implements OnInit {
  ...
  constructor( private _route:ActivatedRoute, private solicitudService: SolicitudService,
  ...
  ngOnInit() {
    ...
    this.solicitudService.getPresupuestoInfo(this.idHEMesualServicio).subscribe(
      (info) => {
        this.informacion = info;
        this.solicitudesHePuesto = this.informacion.puestos; 
        this.solicitudesHePuesto.map(solicitud => {
            solicitud.monto_reservado = solicitud.promedio_salarial * solicitud.horas_autorizadas;
      ...
      },// el false indica que el error se produce al recibir los datos
      (error) =>{
    ...
    this.modalConfirmacionService.confirmar('Por favor confirme..', '¿Realmente desea aprobar el presupuesto de esta solicitud?')
        .then((confirmed) => {
          if (confirmed) {
            ...
            console.log(object);
            this.solicitudService.putPresupuesto(JSON.parse(object)).subscribe(
              ()=>{
                this.toasterManagerService.makeToast('success','Completado',"Se han asignado los montos reservados a cada puesto de la solicitud.")
                this.estado = true;
                this._router.navigate(['/stepi/horas-extra/solicitud/listar-presupuesto']);
              },
